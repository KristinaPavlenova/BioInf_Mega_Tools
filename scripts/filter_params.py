...
def filter_gc(seq: str, gc_bounds: tuple | float | int) -> bool:
...
def filter_length(seq: str, length_bounds: tuple | int) -> bool:
    if not isinstance(length_bounds, tuple):
        length_bounds = (0, length_bounds)
    return length_bounds[0] <= len(seq) <= length_bounds[1]


def filter_quality(scores: str, quality_threshold: float | int) -> bool:
